...
import { getUserInfoRefresh } from '@/store/modules/auth/helpers';
  ...
  if (!route.isInitAuthRoute) {
    ...
    if (!isLogin) {
      ...
      if (route.isValidConstantRoute(toName) && !to.meta.requiresAuth) {
        ...
        // console.log(redirect, '11111111111');
        next({ name: routeName('login'), query: { redirect } });
      }
      return false;
    }
    // const token:ApiAuth.Token=localStg.get('token')
    // 页面刷新初始化所做的操作
    await route.initAuthRoute();
    const userInfoRefreshData: Auth.UserInfo = await new Promise(resovle => {
      getUserInfoRefresh().then(res => {
        resovle(res);
      });
    });
    if (userInfoRefreshData) {
      localStg.set('userInfo', userInfoRefreshData);
      userAuth.$patch({
        userInfo: userInfoRefreshData
      });
    }
