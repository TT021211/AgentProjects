    ...
    <my-card title="搜索条件" search>
      ...
      >
        <n-grid :cols="4" x-gap="15">
          ...
          <n-form-item-grid-item :span="1" label="保养人">
            <n-input v-model:value="searchForm.headName" placeholder="请输入保养人名称"></n-input>
          </n-form-item-grid-item>
          ...
          <n-form-item-grid-item :span="2" label="保养时间">
...
const columns: Ref<DataTableColumns<DataType>> = ref([
  ...
  },
  {
    ...
    key: 'status',
    render: (row: any) => (
      <n-tag type={row.status === '0' ? 'success' : 'error'}>{row.status === '0' ? '已完成' : '未保养'}</n-tag>
    )
  },
  {
    title: '备注',
