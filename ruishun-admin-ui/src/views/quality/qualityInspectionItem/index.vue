<template>
  <div>
    <my-card title="搜索条件" search>
      <n-form inline>
        <n-form-item label="质检检测项编码">
          <n-input v-model:value="searchForm.detectionCode" type="text" placeholder="请输入质检检测项编码" />
        </n-form-item>
        <n-form-item label="质检检测项名称">
          <n-input v-model:value="searchForm.detectionName" type="text" placeholder="请输入质检检测项名称" />
        </n-form-item>
        <n-form-item>
            ...
            <n-grid x-gap="12" :cols="2">
              ...
              <n-form-item-grid-item label="质检检测项编码" path="detectionCode">
                <n-input
                  ...
                  type="text"
                  placeholder="请输入质检检测项编码"
                  :style="{ width: formItemWidth }"
                />
              </n-form-item-grid-item>
              <n-form-item-grid-item label="质检检测项名称" path="detectionName">
                <n-input
                  v-model:value="addForm.detectionName"
                  type="text"
                  placeholder="请输入质检检测项名称"
                  :style="{ width: formItemWidth }"
                />
              </n-form-item-grid-item>
              <n-form-item-grid-item label="是否必检项" path="detectionName">
                <n-select
...
const addForm = ref<qualityInspectionItem.addOrEditData>({
  id: null,
  itemTypeId: null,
  detectionCode: '',
  detectionName: '',
...
const data = ref<qualityInspectionItem.TableList[]>([]);

const columns: Ref<DataTableColumns<qualityInspectionItem.TableList>> = ref([
  {
    title: '序号',
    key: 'index',
    align: 'center',
...
const sleep = id =>
  new Promise(resolve => {
    deleteQcInspectionItems(id).then(res => {
      resolve(res);
    });
  });
...
function addOrEdit(count: number, row?: qualityInspectionItem.TableList): void {
  type.value = count;
  show.value = true;
  if (count === 0) {
    title.value = '新增质检检测项';
...
function submit(): void {
  ...
  formModel.value.validate(errors => {
    if (!errors) {
      ...
      if (type.value === 0) {
        submitSame(addQcInspectionItems, '添加成功');
      } else {
        submitSame(editQcInspectionItems, '编辑成功');
      }
    }
  });
}
...
function changeStatus(row: qualityInspectionItem.TableList, val: string): void {
  row.enable = val;
  const msg = row.enable === 'Y' ? '启用成功' : '已关闭启用';
  editQcInspectionItems(row).then(res => {
    if (res.code === 200) {
      message.success(msg);
    }
  });
...
function init() {
  startLoading();
  getQcInspectionItems(searchForm.value).then(res => {
    endLoading();
    if (res.code === 200) {
      data.value = res.rows;
