...
import {
  insertStrandedwireWorkorder,
  cancelInsertStrandedwireWorkorder,
  insertStrandedwireWorkorderComplete,
  getStrandedwire,
  getStrandedwireWorkorder
} from '~/src/service/api/plan/planStrandedWire';
const { loading, startLoading, endLoading } = useLoading();

const title = ref<string>('绞线计划管理列表');
...
function handleDeleteItemType() {
  insertDialog('取消', cancelInsertStrandedwireWorkorder);
}

function handleDeleteItemTypeComplete() {
  insertDialog('完成', insertStrandedwireWorkorderComplete);
}
...
function submit() {
  ...
  insertStrandedwireWorkorder(addForm.value).then(res => {
...
function handleConfirm(row: planWorkOrder.planWorkOrderList) {
  ...
  getStrandedwire({ equipmentCode, axleNumber, specification, status: 0, pageSize: 999 }).then(res => {
    if (res.code === 200) {
      workOrderData.value = res.rows;
      res.rows.forEach(item => {
        workOrderList.value.push({
          label: item.strandedwireCode,
          value: item.id
        });
      });
    }
...
function insertDetail(row: planWorkOrder.planWorkOrderList) {
  ...
  status.value = '';
  getStrandedwireWorkorder(row.insertId).then(res => {
    if (res.code === 200) {
