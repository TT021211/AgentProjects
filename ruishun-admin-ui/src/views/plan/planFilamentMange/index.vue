...
import {
  getWireWorkorder,
  insertDoublewireWorkorder,
  cancelInsertDoublewireWorkorder,
  insertDoublewireWorkorderComplete,
  getDoublewireWorkorder
} from '~/src/service/api/plan/planFilament';
const { loading, startLoading, endLoading } = useLoading();

const title = ref<string>('并丝计划管理列表');
...
function handleDeleteItemType() {
  insertDialog('取消', cancelInsertDoublewireWorkorder);
}

function handleDeleteItemTypeComplete() {
  insertDialog('完成', insertDoublewireWorkorderComplete);
}
...
function submit() {
  ...
  insertDoublewireWorkorder(addForm.value).then(res => {
...
function handleConfirm(row: planWorkOrder.planWorkOrderList) {
  ...
  getWireWorkorder({ equipmentCode, axleNumber, specification, status: 0, pageSize: 999 }).then(res => {
    if (res.code === 200) {
      workOrderData.value = res.rows;
      res.rows.forEach(item => {
        workOrderList.value.push({
          label: item.doublewireCode,
          value: item.id
        });
      });
    }
...
function insertDetail(row: planWorkOrder.planWorkOrderList) {
  ...
  status.value = '';
  getDoublewireWorkorder(row.insertId).then(res => {
    if (res.code === 200) {
