        ...
        <div>
          <n-form style="width: 900px" :model="formModel" label-placement="left" label-width="90px">
            <n-grid :cols="2" :x-gap="20">
              ...
              <n-form-item-grid-item label="镀锡机台: ">
    ...
    <my-dialog v-model:show="editShow" title="编辑" width="1200px" @cancel="editShow = false" @submit="submitEdit">
      <template #content>
        <div>
          ...
          >
            <n-grid :cols="2" :x-gap="20">
              <n-form-item-grid-item label="镀锡机台" path="equipmentId">
                <n-space>
                  <n-select
            ...
            <n-form-item v-show="editData.equipmentId && editData.axleNumber">
              <div style="overflow: auto; max-height: 350px">
...
import { getWorkTinning, putTinning } from '~/src/service/api/plan/paintJobOrder';
// import {
...
import { getListByTinningId } from '~/src/service/api/plan/productOrder';
import { tinningWorkorderComplete } from '~/src/service/api/produre/workOrderArrange';
...
const title = ref<string>('镀锡工单-全部');
...
function edit(row: paintJobOrder.tabelList) {
  editData.value = row;
  editShow.value = true;
  const index = machineList.value.findIndex((item: any) => item.id === row.equipmentId);
  if (index === -1) return;
  ...
  const axleNumberIds = row.axleNumber ? row.axleNumber.split(',') : [];
  axleNumberId.value = [];
...
function getDetailList() {
  ...
  startLoading();
  if (editData.value.equipmentId === null || editData.value.axleNumber === null) return;
  const params = { equipmentId: editData.value.equipmentId, axleNumber: editData.value.axleNumber };
...
function submitEdit() {
  qbFormModel.value?.validate(errors => {
    if (!errors) {
      putTinning(editData.value).then(res => {
        if (res.code === 200) {
          init();
...
function sleep2(id: number): any {
  return new Promise(resolve => {
    tinningWorkorderComplete(id, '2').then(res => {
      resolve(res);
    });
  });
}
...
function handleUpdateValue(value: number, option: planWorkOrder.SelectMixedOption) {
  ...
  if (craftTitle.value !== '拉丝排产') {
    ...
    locationOptions.value = [];
    drawBenchData.value.axleNumber = null;
...
//       putTinning({ ...putList.value }).then(rs => {
//         if (rs.code === 200) {
//           init();
...
function search() {
  if (searchForm.value.status !== undefined) {
    title.value = `镀锡工单-${workOrderStatus[searchForm.value.status].label}`;
  } else {
    title.value = `镀锡工单-全部`;
  }
  if (range.value !== null) {
