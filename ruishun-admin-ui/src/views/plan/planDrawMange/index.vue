...
import { getEnamellingWorkorderList } from '~/src/service/api/plan';
// import { getEnamellingWorkorder } from '~/src/service/api/plan/enamellingWorkOrder';
const { loading, startLoading, endLoading } = useLoading();

const title = ref<string>('拉丝计划管理列表');
...
function insertDialog(titleText) {
  const d = dialog.info({
    title: `插单${titleText}`,
    ...
    onPositiveClick: () => {
      ...
      return new Promise(resolve => {
        sleep(insertId.value).then((res: any) => {
          if (res.code === 200) {
...
function submit() {
  ...
  // insertWorkorder(addForm.value).then(res => {
...
function handleConfirm(row: planWorkOrder.planWorkOrderList) {
  ...
  getEnamellingWorkorderList({ equipmentCode, axleNumber, specification, status: 0, pageSize: 999 }).then(res => {
    if (res.code === 200) {
      workOrderData.value = res.rows;
      res.rows.forEach(item => {
        workOrderList.value.push({
          label: item.enamellingCode,
          value: item.id
        });
      });
    }
...
function insertDetail(row: planWorkOrder.planWorkOrderList) {
  ...
  status.value = '';
  // getEnamellingWorkorder(row.insertId).then(res => {
  //   if (res.code === 200) {
...
function init() {
  ...
  getEnamellingWorkorderList({ ...searchForm.value }).then(res => {
    if (res.code === 200) {
      ...
      res.rows.push(res.rows[0]);
      res.rows.push(res.rows[0]);
      res.rows.map(n => {
        n.equipmentCode = n.equipmentCode.replace('QB', 'BS');
        n.model = n.model.replace('QA', 'BS');
        n.model = n.model.replace('QZY', 'BS');
        return n;
      });
      data.value = res.rows;
    }
