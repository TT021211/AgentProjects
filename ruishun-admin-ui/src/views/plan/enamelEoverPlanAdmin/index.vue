...
import {
  insertTinningWorkorder,
  cancelInsertTinningWorkorder,
  insertTinningWorkorderComplete,
  getWorkTinning,
  getTinningWorkorder
} from '~/src/service/api/plan/paintJobOrder';
const { loading, startLoading, endLoading } = useLoading();

const title = ref<string>('镀锡计划管理列表');
...
const columns: Ref<DataTableColumns<planWorkOrder.planWorkOrderList>> = ref([
  ...
  },
  {
    ...
    width: 100,
    render: row => {
      return getInsertStatusName(row.insertStatus);
    }
  },
  {
    title: '操作',
...
function handleDeleteItemType() {
  insertDialog('取消', cancelInsertTinningWorkorder);
}

function handleDeleteItemTypeComplete() {
  insertDialog('完成', insertTinningWorkorderComplete);
}
...
function submit() {
  ...
  insertTinningWorkorder(addForm.value).then(res => {
...
function insertDetail(row: planWorkOrder.planWorkOrderList) {
  ...
  status.value = '';
  getTinningWorkorder(row.insertId).then(res => {
    if (res.code === 200) {
