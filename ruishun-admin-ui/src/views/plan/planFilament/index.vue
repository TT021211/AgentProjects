        ...
        <div>
          <n-form style="width: 900px" :model="formModel" label-placement="left" label-width="90px">
            <n-grid :cols="2" :x-gap="20">
              ...
              <n-form-item-grid-item label="并丝机台: ">
...
import { getWireWorkorder, putDoublewire } from '~/src/service/api/plan/planFilament';
import {
...
import { doublewireWorkorderComplete } from '~/src/service/api/produre/workOrderArrange';
...
const title = ref<string>('并丝工单-全部');
...
function submitEdit() {
  qbFormModel.value?.validate(errors => {
    if (!errors) {
      putDoublewire(editData.value).then(res => {
        if (res.code === 200) {
          init();
...
function sleep2(id: number): any {
  return new Promise(resolve => {
    doublewireWorkorderComplete(id, '2').then(res => {
      resolve(res);
    });
  });
}
...
function submitRepetitiom(submitForModel: any, obj: any, func: any) {
  func({
    ...
    if (res.code === 200) {
      ...
      putDoublewire({ ...putList.value }).then(rs => {
        if (rs.code === 200) {
          init();
...
function search() {
  if (searchForm.value.status !== undefined) {
    title.value = `并丝工单-${workOrderStatus[searchForm.value.status].label}`;
  } else {
    title.value = `并丝工单-全部`;
  }
  if (range.value !== null) {
