        ...
        <div>
          <n-form style="width: 900px" :model="formModel" label-placement="left" label-width="90px">
            <n-grid :cols="2" :x-gap="20">
              ...
              <n-form-item-grid-item label="退火机台: ">
    ...
    <my-dialog v-model:show="editShow" title="编辑" width="1200px" @cancel="editShow = false" @submit="submitEdit">
      <template #content>
        <div>
          ...
          >
            <n-grid :cols="2" :x-gap="20">
              <n-form-item-grid-item label="退火机台" path="equipmentId">
                <n-space>
                  <n-select
...
import { getWorkAnnealing, putAnneal } from '~/src/service/api/plan/planAnneal';
// 拉丝
...
import { annealingWorkorderComplete } from '~/src/service/api/produre/workOrderArrange';
...
const title = ref<string>('退火工单-全部');
...
function submitEdit() {
  qbFormModel.value?.validate(errors => {
    if (!errors) {
      putAnneal(editData.value).then(res => {
        if (res.code === 200) {
          init();
...
function sleep2(id: number): any {
  return new Promise(resolve => {
    annealingWorkorderComplete(id, '2').then(res => {
      resolve(res);
    });
  });
}
...
function submitRepetitiom(submitForModel: any, obj: any, func: any) {
  func({
    ...
    if (res.code === 200) {
      ...
      putAnneal({ ...putList.value }).then(rs => {
        if (rs.code === 200) {
          init();
...
function search() {
  if (searchForm.value.status !== undefined) {
    title.value = `退火工单-${workOrderStatus[searchForm.value.status].label}`;
  } else {
    title.value = `退火工单-全部`;
  }
  if (range.value !== null) {
