...
import { getProcureMaterialList } from '~/src/service/api/procure/pushManage';
...
const columns: Ref<DataTableColumns<DataType>> = ref([
  ...
  },
  {
    ...
    key: 'materialType',
    width: 100,
    render: row => typeList.value.find(item => item.value === row.materialType)?.label
  },
  {
    title: '采购编号',
    align: 'center',
...
function init() {
  startLoading();
  getProcureMaterialList(searchForm.value).then(res => {
    endLoading();
    if (res.code === 200) {
      data.value = res.rows;
