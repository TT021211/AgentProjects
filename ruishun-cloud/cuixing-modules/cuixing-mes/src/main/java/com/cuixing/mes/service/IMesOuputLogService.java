package com.cuixing.mes.service;

import com.cuixing.mes.entity.MesOuputLog;
import com.cuixing.mes.entity.vo.MesOutputLogVo;

import java.math.BigInteger;
import java.util.List;

/**
 * 报工记录表;(mes_ouput_log)表服务接口
 * @author : cuixing-keyun-auto
 * @date : 2024-4-12
 */
public interface IMesOuputLogService{
    /**
     * 通过ID查询单条数据
     *
     * @param id 主键
     * @return 实例对象
     */
    MesOuputLog queryById(BigInteger id);
    /**
     * 分页查询
     *
     * @param mesOuputLog 筛选条件
     * @return 查询结果
     */
    List<MesOuputLog> selectMesOuputLogList(MesOuputLog mesOuputLog);
    /**
     * 新增数据
     *
     * @param mesOuputLogs 实例对象
     * @return 影响行数
     */
    List<MesOuputLog> insert(List<MesOuputLog> mesOuputLogs);

    /**
     * 批量新增数据
     *
     * @param mesOuputLogs 实例对象数组
     * @return 影响行数
     */
    int insertBatch(List<MesOuputLog> mesOuputLogs);
    /**
     * 更新数据
     *
     * @param mesOuputLog 实例对象
     * @return 影响行数
     */
    int update(MesOuputLog mesOuputLog);

    int updateInfo(MesOuputLog mesOuputLog);
    /**
     * 通过主键删除数据
     *
     * @param id 主键
     * @return 是否成功
     */
    boolean deleteById(BigInteger id);
    /**
     * 通过主键批量删除数据
     *
     * @param ids 主键
     * @return 是否成功
     */
    boolean deleteByIds(BigInteger[] ids);

    List<MesOuputLog> WorkorderComplete(List<MesOuputLog> mesOuputLogs);

    MesOuputLog getInfoByOutputSn(String outputSn);

    List<MesOuputLog> selectMesOuputLogWmsList(MesOuputLog mesOuputLog);

    MesOutputLogVo selectWmsmesOutputLogListSum(MesOuputLog mesOuputLog);

    MesOuputLog getSpecificationByWorkstationCode(String workstationCode);

    List<MesOuputLog> selectByTime(String taskType);

    boolean scrapped(MesOuputLog mesOuputLog);
}