package com.cuixing.md.mapper;

import org.apache.ibatis.annotations.Mapper;
import java.math.BigInteger;
import java.util.List;
import com.cuixing.md.entity.MdPlantOutinLog;
import org.apache.ibatis.annotations.Param;

/**
 * 油漆出入库记录;(md_plant_outin_log)表数据库访问层
 * @author : http://www.chiner.pro
 * @date : 2024-5-31
 */
@Mapper
public interface MdPlantOutinLogMapper{
    /** 
     * 通过ID查询单条数据 
     *
     * @param id 主键
     * @return 实例对象
     */
    MdPlantOutinLog queryById(BigInteger id);
    /** 
     * 分页查询指定行数据
     *
     * @param mdPlantOutinLog 查询条件
     * @return 对象列表
     */
    List<MdPlantOutinLog> selectMdPlantOutinLogList(MdPlantOutinLog mdPlantOutinLog);
    /** 
     * 新增数据
     *
     * @param mdPlantOutinLog 实例对象
     * @return 影响行数
     */
    int insert(MdPlantOutinLog mdPlantOutinLog);
    
    /** 
     * 更新数据
     *
     * @param mdPlantOutinLog 实例对象
     * @return 影响行数
     */
    int update(MdPlantOutinLog mdPlantOutinLog);
    /** 
     * 通过主键批量删除数据
     *
     * @param ids 主键
     * @return 影响行数
     */
    int deleteByIds(BigInteger[] ids);

    MdPlantOutinLog selectByPlantCode(@Param("plantCode") String plantCode);
 }