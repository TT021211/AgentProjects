 ...
 * @date 2024-07-24
 */
public interface EcmElectricDeviceMapper 
{
    /**
     * 查询能耗设备
     * 
     ...
     * 批量删除能耗设备
     * 
     * @param electricDeviceIds 需要删除的数据主键集合
     * @return 结果
     */
    public int deleteEcmElectricDeviceByElectricDeviceIds(Long[] electricDeviceIds);

    List<EcmElectricDevice> getDeviceListByLocation(@Param("location") String location);

    List<EcmElectricDevice> getDeviceListByParentId(Integer parentId);

    EcmElectricDevice getDeviceListByElectricDeviceCode(@Param("electricDeviceCode") String electricDeviceCode);
}
